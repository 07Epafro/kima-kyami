...
export default function DecisaoPagamento({ pagamentoId, estadoActual }: Props) {
  ...
  return (
    <div className="rounded-xl border border-gray-100 bg-white p-6 space-y-4">
      ...
      {erro && (
        <p className="text-xs text-red-600 bg-red-50 px-3 py-2 rounded font-sans">
          {erro}
        </p>
      )}
      ...
      {!mostrarRejeitar ? (
        <div className="flex gap-3">
          <button
            ...
            onClick={confirmar}
            disabled={loading}
            className="flex-1 min-h-11 py-2.5 bg-emerald-600 text-white text-sm rounded hover:bg-emerald-700 transition-colors disabled:opacity-50 font-sans"
          >
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 min-h-11 py-2.5 bg-white border border-red-300 text-red-600 text-sm rounded hover:bg-red-50 transition-colors disabled:opacity-50 font-sans"
          >
            Rejeitar
          </button>
        </div>
        ...
        <div className="space-y-3">
          <div>
            <label
              className="block text-xs text-muted mb-1 font-sans"
            >
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-2 border border-gray-200 rounded text-sm focus:outline-none focus:border-red-400 resize-none font-sans"
            />
          </div>
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 min-h-11 py-2.5 bg-red-600 text-white text-sm rounded hover:bg-red-700 transition-colors disabled:opacity-50 font-sans"
            >
            ...
            <button
              ...
              disabled={loading}
              className="px-4 min-h-11 py-2.5 text-sm text-muted hover:text-noir transition-colors disabled:opacity-50 font-sans"
            >
              Cancelar
            </button>
          </div>
